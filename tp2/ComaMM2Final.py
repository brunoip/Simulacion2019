...
miServer1 = simpleServer()
miServer1.serverName = "1"
miServer1.initialize("1", 2, .6, True)

miServer2 = simpleServer()
miServer2.serverName = "2"
miServer2.initialize("2", 2, .6, True)

miServer3 = simpleServer()
miServer3.serverName = "3"
miServer3.initialize("3", 2, .6, True)
...
def arrive():
    ...
    Client().registerArrival(arriavalTime)
    ...
    if (server_status_1 == BUSY and server_status_2 == BUSY):
        ...
        Client().registerFirstDepartureTime(simulation_time, departureTime, "a")
...
def depart_1():
    ...
    if num_in_q == 0:
        ...
        departureTime = simulation_time + expon(mean_service)
        time_next_event[EVENT_DEPART_1] = departureTime
        Client().registerFirstDepartureTime(time_arrival[len(time_arrival)-1], departureTime, "d")
...
def report_time_in_system():
    ...
    for cliente in Client().clientList:
    ...
    return average_time_spent_in_system / len(clientesConTiempo)
